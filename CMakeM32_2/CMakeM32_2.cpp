...
auto upper_case = [](std::string str) {
    std::transform(str.begin(), str.end(), str.begin(), ::toupper);
    return str;
};

std::vector<std::pair<std::string_view, std::string_view>>
filmography(const std::string& name, const std::vector<Movie>& movies) {
    ...
    for (const auto& movie : movies) {
        ...
        auto it = movie.starring.find(name);
        if (it != movie.starring.end()) { 
            result.push_back({ movie.names.at("English"), it->second });
        }
    }
...
int main() {
    ...
    if (file_read.is_open()) {
        ...
        for (const auto& item : movie_catalogue.items()) {
            ...
            std::cout << "Movie: \"" << item.key() << "\"\n";

            nlohmann::json movie_json = item.value();

            if (movie_json.count("names") > 0) {
                mov.names = movie_json["names"];
            }
            mov.names["English"] = item.key();

            mov.countries = movie_json["countries"];
            mov.release_year = movie_json["release_year"];
            mov.running_time = movie_json["running_time"];
            mov.director = movie_json["director"];
            mov.screenwriter = movie_json["screenwriter"];
            mov.producer = movie_json["producer"];
            mov.starring = movie_json["starring"];
            mov.cinematography = movie_json["cinematography"];
            mov.editor = movie_json["editor"];
            mov.composer = movie_json["composer"];
            mov.production_companies = movie_json["production_companies"];
        ...
        std::string input = "";
        while (true) {
            std::cout << "Enter the actor's name or type \"exit\" for exit.\n";
            ...
            auto result = filmography(input, movies);

            if (result.size() < 1) {
                std::cout << input << " is not found in this catalogue.\n";
                continue;
            }

            for (const auto& mov : result) {
                std::cout << "Plays in film : \"" << mov.first << "\" as " << mov.second << "\n";
            }
        }
